...
//gas_pressure_dis = new TF1("gasP","gas_pressure1(x)",-5,5);
  ...
  if(allowVertexSpread)
      ...
      if(sigmaVertexY > 0.)
	{
      ...
      //vertexX = tmpVtxX;
      vertexY = tmpVtxY;
      vertexZ = tmpVtxZ + random_s1()*1000.0 ; ///add vertex Z for the beam gas pressure
      vertexX = tmpVtxX+vertexZ*(1.0)*TMath::Tan(mXAngle);
    }
  ...
  if(allowMomentumSpread)
    {
      ...
      deltaPyA += (mIonBeamEnergy + tmpPzA)*TMath::Sin(mXAngleY); //qiao
  ...
  if(allowMomentumSpread)
      ...
      if(mDivAcc == 3)
	{
      ...
      double crabKickLep = 0;//= (-1.0)*(mLeptonBeamEnergy + tmpPzB)*TMath::Sin(crabAngLep); // 
